...
class PpdbController extends Controller
{
    ...
    public function index()
    {
        ...
        $timelines = PpdbTimeline::where('is_active', true)
            ->orderBy('order', 'asc')
            ->get();

        // Get active info cards
        $infoCards = PpdbInfoCard::where('is_active', true)
            ->orderBy('order', 'asc')
            ->get();
        ...
        return Inertia::render('Ppdb/Index', [
            ...
            'ppdbSettings' => $ppdbSettings ? [
                ...
                'brochure' => $ppdbSettings->brochure_file ? [
                    'file' => asset('storage/' . $ppdbSettings->brochure_file),
                    'title' => $ppdbSettings->brochure_title,
                    'description' => $ppdbSettings->brochure_description,
                    'extension' => pathinfo($ppdbSettings->brochure_file, PATHINFO_EXTENSION),
                ] : null,
            ] : [
                'isOpen' => false
            ],
    ...
    /**
     * Download SPMB brochure
     */
    public function downloadBrochure()
    {
        $settings = PpdbSetting::current();

        if (!$settings || !$settings->brochure_file) {
            abort(404, 'Brosur tidak ditemukan');
        }

        $filePath = storage_path('app/public/' . $settings->brochure_file);

        if (!file_exists($filePath)) {
            abort(404, 'File brosur tidak ditemukan');
        }

        $filename = $settings->brochure_title
            ? Str::slug($settings->brochure_title) . '.' . pathinfo($settings->brochure_file, PATHINFO_EXTENSION)
            : basename($settings->brochure_file);
